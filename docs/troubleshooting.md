...
**12. 对话中未直接显示工具卡片（只看到最终回复或耗时摘要条）**

- **症状**：模型调用了 Zotero 工具，但在 Chat 消息流中只看到最终答案，或一条带耗时与活动描述的过程摘要栏（如「已调用工具 · 2.1s」），未直接看到展开的工具卡片
- **原因**：DSH 通用设置中的「工作步骤展示」默认为「标准」模式，在轮次正常结束后会自动收起过程行以保持对话清爽
- **处理**：
  - 点击消息上方带时钟图标的过程摘要条，即可就地展开查看所有专属工具卡片
  - 若希望工具卡片在历史与流式生成中始终默认展开，可在 **设置 → 通用设置 → 工作步骤展示**（`Settings → General → Work details`）中选择 **「完全展开」**（`verbose`）
