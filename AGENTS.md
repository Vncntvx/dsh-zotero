...
The DSH packages you launch above come from the sibling pnpm workspace at `../deepseek-harness` (`0.1.3-alpha.1`). `package.json` pins the same `0.1.3-alpha.1` versions; `node_modules` catches up via `npm install` once the registry publishes that line (until then it still holds the last published set, so typecheck/tests run with no skew only after the install). If the sibling moves ahead, run `npm install` to re-align.
...
- `src/client/components/SourcesTab.tsx` reads the tab's call blocks through the modern projection: `ChatSnapshot.order` walked with `nodes.get(key)`, narrowed to `kind: 'tool-call'` rows whose `data.root` carries the full call lifecycle (the visible-only filter matches what the harness's `.legacy` compatibility projection contributes). This is the client-half seam to re-check on any harness upgrade: the `tool-call` row kind and its `ToolChatData.root` shape in ui-chat's `ChatNodeDataMap`, and the visible-only convention. Nothing else consumes tool-call rows.
- The host manifest (`src/typert.ts`) self-registers through `ctx.inject(['typert'], ...)` instead of a `./typert` package export: dsh-typert-loader's auto-discovery resolves `<entry-name>/package.json` from the config-tree anchor, so it only reaches bare-package-name rows — the dev-overlay row spells an absolute file path and would silently lose the manifest, while a `./typert` export would double-register on the production profile where the plugin also self-registers. Re-check both loader behaviors if upstream changes how entries resolve to packages.
- `discovery-smoke.mjs` rides the web-auth seam: it exchanges the boot token printed by `dsh web` for the page cookie and carries that cookie on every probe, and it matches the `globalThis["__DSH_BOOT__"]` injection spelling. Re-verified at 0.1.3-alpha.1 (the `window.` spelling is only a browser-side read alias); re-check on a harness upgrade that touches webserver auth or the boot injection.
