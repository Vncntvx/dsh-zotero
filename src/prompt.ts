...
export const WRITE_POLICY_SENTENCE =
  'When writing (zotero_create_note, zotero_add_tags, zotero_add_to_collection): write note bodies in markdown — the plugin converts them to Zotero note HTML and escapes unknown syntax, so raw HTML never passes through; cite sources by their refs. Child notes never take collections (non-empty collections on a child-note call are refused before any plan). When writeConfirm is on (the default) the write first shows a plan the user approves; kind "declined" means the user did not approve (including dismissing the plan review to talk) — stop, do not retry. ZOTERO_WRITE_CONFLICT means the item changed underneath the read — re-run the tool once, it re-reads and reapplies; ZOTERO_WRITE_UNAUTHORIZED means Zotero refused the write authorization — stop and ask; ZOTERO_WRITE_APPROVAL_UNAVAILABLE means plan-review could not be asked at all — stop, and run where user questions can answer.'
