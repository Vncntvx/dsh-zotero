...
function zoteroPromptTextOf(
  ...
  return [
    ...
    "Workflow: zotero_browse discovers libraries/collections/saved searches/tags/itemTypes; zotero_search discovers candidate papers and returns stable refs of the form zotero://user/0/item/<KEY> or zotero://group/<ID>/item/<KEY>; zotero_get reads one item's metadata (including relations); zotero_retrieve ranks evidence passages (annotations, notes, the abstract, full text) against a query; zotero_attachment resolves an item or attachment ref to a verified on-disk path or a linked URL; zotero_export produces citations, a joined bibliography, or bibtex/biblatex/ris/csljson output. Reuse returned refs — never invent them.",
    `Tool caps (set in the Zotero settings): zotero_search limit up to ${config.maxSearchResults}, zotero_retrieve passages up to ${config.maxEvidencePassages}, zotero_browse limit up to ${config.maxBrowseResults}, and zotero_export refs up to ${config.maxExportRefs}. Exceeding a cap errors, so choose limits within these bounds. On the first result page (offset 0), note-body matches merge up to the limit (up to ${config.maxNoteScanRecords} notes scanned) and are reported in noteMatches, outside the paged total.`,
