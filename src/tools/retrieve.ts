...
const ALL_SOURCES: ('annotation' | 'note' | 'abstract' | 'fulltext')[] = ['annotation', 'note', 'abstract', 'fulltext']

const RETRIEVE_PARAMETERS = {
  ref: { type: 'string', required: true, description: 'A zotero://user/0/item/<KEY> ref from zotero_search or zotero_get.' },
  query: { type: 'string', required: true, description: 'Terms the evidence passages are ranked against.' },
  sources: {
    type: 'array',
  ...
  passages: { type: 'integer', default: 4, description: 'Maximum ranked evidence passages to return.' },
} as const
...
function buildRequest(args: RetrieveArgs, config: ResolvedConfig): ZoteroRetrieveRequest {
  ...
  if (!Number.isInteger(passages) || passages < 1 || passages > config.maxEvidencePassages) {
    invalid(`passages must be an integer between 1 and ${config.maxEvidencePassages}; got ${passages}`)
  }
...
export function renderRetrieve(_args: RetrieveArgs, value: RetrieveOutput): ContentBlock[] {
  const lines = [`Evidence for ${value.ref} (${value.evidence.length} passage${value.evidence.length === 1 ? '' : 's'})`]
  if (value.attachmentRef !== undefined) lines.push(`Full text: ${value.attachmentRef}`)
  if (value.coverage !== undefined) {
    const coverage = value.coverage
    const chars = coverage.indexedChars === undefined ? '' : `${coverage.indexedChars}/${coverage.totalChars ?? '?'} chars`
    const pages = coverage.indexedPages === undefined ? '' : `, ${coverage.indexedPages}/${coverage.totalPages ?? '?'} pages`
    lines.push(`Indexing coverage: ${chars}${pages}${coverage.complete ? ' (complete)' : ''}`)
  }
  value.evidence.forEach((entry) => {
    const label = entry.pageLabel === undefined ? entry.source : `${entry.source} (page ${entry.pageLabel})`
...
export function registerRetrieveTool(ctx: Context, service: ZoteroService, config: ResolvedConfig): void {
